...
`--odic-config-id` には、先程実行した `rosa create oidc-config` コマンドの出力結果に表示されたoidc-config-id (例:0ikrpnt0nv000c000trn00renuakl0mn) を指定します。
...
OCMを使用すると、単一のダッシュボードからすべてのOpenShiftクラスターを操作できるようになります。
https://console.redhat.com/openshift[OpenShift Hybrid Console]
...
$ rosa create idp --type=htpasswd --name=testuser-htpasswd01 --cluster=$CLUSTER_NAME --from-file=users.htpasswd
